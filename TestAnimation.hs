...
pic :: Animation
pic = vortex `plus` star `plus` tardis `plus` doctorWho
...
vortex :: Animation
vortex = translate (always (0,0)) --background
       ...
       (withoutPaint
          (combine
            [translate (cycleSmooth 1 [(300, 300), (300, 200), (500, 300), (500, 200), (400, 300)])
              (circle (repeatSmooth 1 [(1 + i, 1), (2 + j, 700)])) | i <- [0,0.25..1.025], j <- [0, 0.25.. 1.025]]))
    `plus`
    translate 
...
test :: IO ()
test = writeFile "animationTest.svg" (svg 800 600 pic)